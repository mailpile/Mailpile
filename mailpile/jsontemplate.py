...
SIMPLE_FUNC, ENHANCED_FUNC, DUAL_FUNC = 0, 1, 2
...
class _ProgramBuilder(object):
  ...
  def AppendPartial(self, name):
    # Boots up new parser instance, appends the tree generated by that instance to the current tree.
    # Question: Do we want this to be lazy?
    try:
      name = os.path.abspath(name)
      template_str = open(name).read()
      partialroot = CompileTemplate(template_str)
      partialroot.section_name = name
      self._NewSection(_DoSection, partialroot)
    except Exception, e:
      self.current_block.Append("Error adding partial: %s" % e)
  ...
  def NewEqualityPredicateSection(self, t1, t2, cmpr):
    if cmpr == "==":   pred = (lambda x,y: x == y, (t1, t2), 2)
    elif cmpr == "!=": pred = (lambda x,y: x != y, (t1, t2), 2)
    elif cmpr == "<": pred = (lambda x,y: x < y, (t1, t2), 2)
    elif cmpr == "<=": pred = (lambda x,y: x <= y, (t1, t2), 2)
    elif cmpr == ">=": pred = (lambda x,y: x >= y, (t1, t2), 2)
    elif cmpr == ">": pred = (lambda x,y: x > y, (t1, t2), 2)
...
( LITERAL_TOKEN,  # "Hi"
  ...
  COMPLEX_PREDICATE_TOKEN, # {.if predicate}
  ALTERNATES_TOKEN,  # {.or}
  OR_TOKEN,  # {.or}
  END_TOKEN,  # {.end}
...
def _MatchDirective(token):
  ...
  if token.startswith('if '):
    return COMPLEX_PREDICATE_TOKEN, token[3:].strip()
  if token.endswith('?'):
    return PREDICATE_TOKEN, token

  if token.startswith('partial '):
    return PARTIAL_TOKEN, token[8:].strip()
...
def CompileTemplate(
  ...
  for token_type, token in _Tokenize(template_str, meta_left, meta_right):
    ...
    if token_type == COMPLEX_PREDICATE_TOKEN:
      m = re.match("(.*) (==|!=|\<|\>|\<=|\>=) (.*)", token)
      if m:
        t1, cmpr, t2 = m.groups()
        builder.NewEqualityPredicateSection(t1.strip(), t2.strip(), cmpr)
      else:
        block_made = builder.NewPredicateSection(token)
      balance_counter += 1
      continue
    ...
    if token_type == PARTIAL_TOKEN:
      builder.AppendPartial(token)
    ...
    if func_type == DUAL_FUNC:
      ...
      for i in range(2):
        if args[i].isdigit():
          ...
          vals[i] = context.Lookup(args[i])
      do_clause = predicate(*vals)
    elif func_type == ENHANCED_FUNC:
...
def _DoPartial(args, context, callback):
  """Include partials"""
  filename = args
  # FIXME
  # callback()
