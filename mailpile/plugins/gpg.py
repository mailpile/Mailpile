...
class GPG(Command):
        ...
        try:
            ...
            for line in keylines:
                if line[0:3] == "pub":
                    ...
                    curkey["uids"] = []
                    curkey["pub"] = {
                        "keyid": keyid, "type": keytype, "created": created, "expires": expiry}
                elif line[0:3] == "sec":
                    if curkey != {}:
                        keys.append(curkey)
                        curkey = {}
    ...
    def fingerprints(self):
        ...
        try:
            session.ui.mark('Listing available GPG keys')
            gpg = GnuPG().run(
                ['--fingerprint', arg], create_fhs=['stderr', 'stdout'])
            keylines = gpg.handles['stdout'].readlines()
    ...
    def command(self):
        ...
        session.ui.mark(
            "GPG Encryption interface. Use subcommands to interact:\n\n")
