...
class SearchResults(dict):
    ...
    def _names(self, senders):
        try:
            if len(senders) > 1:
                text = re.sub(
                    '["<>]', '', ', '.join([x.split()[0] for x in senders]))
                return text
        except IndexError:
            pass
    ...
    def __init__(self, session, idx,
        ...
        for mid in results[start:start + num]:
            ...
            result = self._explain_msg_summary([
                                               ...
                                               msg_info[
                                                   MailIndex.MSG_TAGS].split(
                                                   ','),
                                               session.config.is_editable_message(
                                                   msg_info[MailIndex.MSG_PTRS])
                                               ])
    ...
    def as_text(self):
        ...
        for m in self['messages']:
            if 'message' in m:
                ...
                text.append(
                    exp_email.get_editing_string(exp_email.get_message_tree()))
            else:
...
class View(Search):
    ...
    def command(self):
        ...
        idx.apply_filters(
            session, '@read', msg_idxs=[e.msg_idx for e in emails])
        for email in emails:
            if raw:
                results.append(
...
class Extract(Command):
    ...
    class CommandResult(Command.CommandResult):
        ...
        def _fixup(self):
            ...
            for result in (self.result or []):
                if 'data' in result:
                    result['data'] = result['data'].encode(
                        'base64').replace('\n', '')
            self.fixed_up = True
            return self
