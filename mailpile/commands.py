            ...
            if what.lower() == 'these':
                ...
                try:
                    ...
                    if msg_id >= 0 and msg_id < len(self._idx().INDEX):
                        ...
                        self.session.ui.warning(
                            'ID out of bounds: %s' % (what[1:], ))
                except ValueError:
        ...
        try:
            ...
            for fid, fpath in config.get_mailboxes():
                if mailpile.util.QUITTING:
                    break
                count += idx.scan_mailbox(
                    session, fid, fpath, config.open_mailbox)
...
class AddMailbox(Command):
    ...
    def command(self):
        session, config, raw_fn = self.session, self.session.config, self.args[
            0]
...
class Help(Command):
    ...
    class CommandResult(Command.CommandResult):
        ...
        def splash_as_text(self):
            return '\n'.join([
                             self.result['splash'],
                             'The web interface is %s' % (
                                 self.result['http_url'] or 'disabled.'),
                             '',
        ...
        if self.args:
            ...
            for grp in COMMAND_GROUPS:
                ...
                for c in COMMANDS:
                    ...
                    if cls.ORDER[0] == grp:
                        cmd_list[c] = (
                            name, synopsis, cls.__doc__, count + cls.ORDER[1])
            return {
                'commands': cmd_list,
