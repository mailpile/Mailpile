            ...
            for fn in sorted(os.listdir(postinglist_dir)):
                ...
                if (force
                   ...
                   900 * postinglist_kb):
                    ...
                    # FIXME: Remove invalid and deleted messages from posting
                    # lists.
    ...
    def save(self, prefix=None, compact=True, mode='w'):
        ...
        try:
            ...
            self.session.ui.mark(
                'Writing %d bytes to %s' % (len(output), outfile))
            if output:
                try:
...
class GlobalPostingList(PostingList):
    ...
    def Append(cls, session, word, mail_ids, compact=True):
        super(GlobalPostingList, cls).Append(
            session, word, mail_ids, compact=compact)
...
class MailIndex(object):
    ...
    def load(self, session=None):
        ...
        if session:
            session.ui.mark(
                'Loaded metadata for %d messages' % len(self.INDEX))
    ...
    def scan_mailbox(self, session, idx, mailbox_fn, mailbox_opener):
        try:
            ...
            session.ui.mark(
                '%s: Error opening: %s (%s)' % (idx, mailbox_fn, e))
            return 0
            ...
            if msg_id in self.MSGIDS:
                ...
                try:
                    ...
                    if msg_date > (time.time() + 24 * 3600):
                        session.ui.warning(
                            '=%s/%s is from the FUTURE!' % (msg_mid, msg_id))
                        # Messages from the future are treated as today's
                        msg_date = last_date + 1
                    elif msg_date < 1:
                        session.ui.warning(
                            '=%s/%s is PREHISTORIC!' % (msg_mid, msg_id))
                        msg_date = last_date + 1

                except (ValueError, TypeError, OverflowError):
                    session.ui.warning(
                        '=%s/%s has a bogus date.' % (msg_mid, msg_id))
                ...
                keywords = self.index_message(
                    session, msg_mid, msg_id, msg, msg_date,
                    mailbox=idx, compact=False,
                    filter_hooks=[self.filter_keywords])
                tags = [k.split(':')[0]
                        for k in keywords if k.endswith(':tag')]
    ...
    def message_keywords(self, session, msg_mid, msg_id, msg, msg_date,
        ...
        for part in msg.walk():
            ...
            def _loader(p):
                if payload[0] is None:
                    payload[0] = self.try_decode(
                        p.get_payload(None, True), charset)
                return payload[0]
            if ctype == 'text/plain':
            ...
            if textpart:
                ...
                for extract in plugins.get_text_kw_extractors():
                    keywords.extend(
                        extract(self, msg, ctype, lambda: textpart))
        ...
        keywords.extend(
            re.findall(WORD_REGEXP, self.hdr(msg, 'subject').lower()))
        ...
        for key in msg.keys():
            ...
            if key_lower not in BORING_HEADERS:
                ...
                words = set(
                    re.findall(WORD_REGEXP, self.hdr(msg, key).lower()))
                words -= STOPLIST
    ...
    def index_message(self, session, msg_mid, msg_id, msg, msg_date,
        ...
        keywords = self.message_keywords(
            session, msg_mid, msg_id, msg, msg_date,
            mailbox=mailbox)
        ...
        for word in keywords:
            try:
                GlobalPostingList.Append(
                    session, word, [msg_mid], compact=compact)
    ...
    def search(self, session, searchterms, keywords=None):
        ...
        for p in ('', '+', '-'):
            ...
            while p + 'in:trash' in searchterms:
                searchterms[
                    searchterms.index(p + 'in:trash')] = p + 'tag:Trash'
    ...
    def sort_results(self, session, results, how=None):
        ...
        try:
            if how == 'unsorted':
                ...
                results.sort(
                    key=lambda k: long(self.get_msg_by_idx(k)[self.MSG_DATE], 36))
            elif how.endswith('from'):
                results.sort(
                    key=lambda k: self.get_msg_by_idx(k)[self.MSG_FROM])
            elif how.endswith('subject'):
                results.sort(
                    key=lambda k: self.get_msg_by_idx(k)[self.MSG_SUBJECT])
            else:
