...
class xxBaseUI(object):
    ...
    def warning(self, message):
        self.say('Warning: %s%s' %
                 (message, ' ' * (self.WIDTH - 11 - len(message))))

    def error(self, message):
        self.say('Error: %s%s' %
                 (message, ' ' * (self.WIDTH - 9 - len(message))))
                ...
                if context:
                    try:
                        try:
                            ...
                            self.warning(
                                'Bad conversation: %s' % tree['conversation'])
                            conversation = [email.msg_idx]
                        self.display_results(email.index,  conversation, [],
                                             expand=[email], fd=fd)
                    except TypeError:
                        self.warning(
                            'No conversation, bad ID: %s' % email.msg_idx)
...
class xxTextUI(xxBaseUI):
    ...
    def reset_marks(self, quiet=False):
        ...
        if t:
            if not quiet:
                ...
                self.say('%s%s' %
                         (result, ' ' * (self.WIDTH - 1 - len(result))))
            return t[-1][0] - t[0][0]
        else:
            return 0
    ...
    def mark(self, progress):
        self.say(
            '  %s%s\r' % (progress, ' ' * (self.WIDTH - 3 - len(progress))),
        ...
        for mid in results[start:start + num]:
            ...
            if expand and mid in expand_ids:
                ...
                try:
                    ...
                    sfmt = '%%-%d.%ds%%s' % (
                        41 - (clen + len(msg_tags)), 41 - (clen + len(msg_tags)))
            ...
            try:
                if email.is_editable():
                    ...
                    if es != ns:
                        ...
                        self.say(
                            'Message saved.  Use the "mail" command to send it.')
                    else:
...
class RawHttpResponder:
    ...
    def __init__(self, request, attributes={}):
        ...
        filename = attributes.get(
            'filename', 'attachment.dat').replace('"', '')
...
class xxJsonUI(xxHttpUI):
    ...
    def render(self, session, request_url, request_path):
        message, mimetype = self.render_data(
            session, request_url, request_path)
        ...
        self.request.send_standard_headers(
            mimetype=mimetype, cachectrl='no-cache')
        self.request.wfile.write(message)
        self.request.log_request(
            self.status_code, message and len(message) or '-')
...
class xxXmlUI(xxJsonUI):
    ...
    def render_xml_data(self, data, name='', attrs={}, indent=''):
        ...
        if type(data) == type(dict()):
            ...
            data = self.render_xml_list(
                data, name=name, indent=indent) + indent
            dtype = 'list'
            if self.BARE_LISTS:
                return data
        elif type(data) == type(set()):
            data = self.render_xml_list(
                list(data), name=name, indent=indent) + indent
            dtype = 'set'
            if self.BARE_LISTS:
                return data
        else:
    ...
    def render_xml_list(self, lst, name='items', indent=''):
        ...
        if name.endswith('s'):
            ...
            nh = 'item'
        for item in lst:
            xml.append(self.render_xml_data(
                item, name=nh, indent=indent + ' '))
        return '\n'.join(xml) + '\n'
    ...
    def render_xml_dict(self, dct, name='dict', indent=''):
        ...
        for name in dct.keys():
            xml.append(self.render_xml_data(
                dct[name], name=name, indent=indent + ' '))
        return '\n'.join(xml) + '\n'
...
class xxRssUI(xxXmlUI):
    ...
    def prune_message_tree(self, tree):
        r = {}
        r['items'] = [self.explain_msg_summary(c)
                                               for c in tree['conversation']]
        for item in r['items']:
            if item['_id'] == tree['id']:
                item[
                    'description'] = 'FIXME: Insert text body here, w/o quotes?'
        return r
    ...
    def render_data(self, session, request_url, request_path):
        ...
        return (
            xxXmlUI.render_data(self, session, request_url, request_path)[0],
                'application/rss+xml')
...
class xxHtmlUI(xxHttpUI):
    ...
    def transform_text(self):
        ...
        self.buffered_html.append(
            ('html', '<pre id="loglines">%s</pre>' % ''.join(text)))
    ...
    def render_page(self, config, variables, body='', title='', sidebar=''):
        tpl = config.get('path', {}).get(
            self.request.http_host(), 'html_template')
    ...
    def display_results(self, idx, results, terms,
        ...
        self.buffered_html.append(
            ('html', '<table class="results" id="results">\n'))
        expand_ids = [e.msg_idx for e in (expand or [])]
        for mid in results[start:start + num]:
            count += 1
            try:
                ...
                if expand and mid in expand_ids:
                    ...
                    msg_date = datetime.date.fromtimestamp(
                        int(msg_info[idx.MSG_DATE], 36))
