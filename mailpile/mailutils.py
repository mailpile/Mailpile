...
def HeaderPrint(message):
    ...
    # [x.split(':', 1)[0] for x in message.raw_header]
...
def OpenMailbox(fn):
    if fn.startswith("imap://"):
        # FIXME(halldor): waaaayy too naive - expects
...
class Email(object):
    ...
    def Create(cls, idx, mbx_id, mbx,
        ...
        if msg_to:
            msg['To'] = cls.encoded_hdr(
                None, 'to', value=', '.join(set(msg_to)))
        if msg_cc:
            msg['Cc'] = cls.encoded_hdr(
                None, 'cc', value=', '.join(set(msg_cc)))
        if msg_bcc:
            msg['Bcc'] = cls.encoded_hdr(
                None, 'bcc', value=', '.join(set(msg_bcc)))
        if msg_references:
        ...
        if attachments:
            ...
            for att in oldtree['attachments']:
                ...
                if hdr in attachments:
                    # FIXME: Update the filename to match whatever the user
                    # typed
            ...
            if (part.get('content-disposition', 'inline') == 'inline'
               and mimetype in ('text/plain', 'text/html')):
                ...
                if (mimetype == 'text/html' or
                    ...
                    tree[
                        'text_parts'].extend(self.parse_text_part(payload, charset,
                                                                  openpgp))
            else:
                ...
                if part['type'] in ('pgpbegin', 'pgptext'):
                    ...
                    try:
                        ...
                        for fh in ('stderr', 'stdout'):
                            ...
                            gpg.handles[fh].close()
                        gpg.wait()
                        pgpdata[0]['data'] = results[
                            'stderr'].decode('utf-8') + '\n'
                        pgpdata[1]['data'] = self._decode_gpg(
