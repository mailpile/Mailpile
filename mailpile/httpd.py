...
class HttpRequestHandler(SimpleXMLRPCRequestHandler):
    ...
    def send_full_response(
        self, message, code=200, msg='OK', mimetype='text/html',
        ...
        if '..' in filename:
            ...
            try:
                tpl = config.get('path', {}).get(
                    self.http_host(), 'html_template')
                fpath, fd = config.open_file(tpl, filename)
                mimetype = mimetypes.guess_type(
                    fpath)[0] or "application/octet-stream"
                message = fd.read()
                code, msg = 200, "OK"
            except IOError, e:
                if e.errno == 2:
    ...
    def parse_pqp(self, path, query_data, post_data, config):
        q = post_data.get('lq', query_data.get('q', ['']))[
            0].strip().decode('utf-8')

        data = {}
        cmd = ''
        if path.startswith('/_/'):
            cmd = path[3:].replace('/', ' ').replace(
                '.json', '').replace('.jhtml', '').replace('.xml', '')
            ...
            if len(parts) == 4:
                ...
                if parts[3] in ('inline', 'inline-preview', 'preview', 'download'):
                    cmd = ' '.join(
                        ['extract', parts[3], '#%s' % parts[4], parts[1]])
        ...
        try:
            ...
            if cmd:
                for arg in cmd.split(' /'):
                    ...
                    result = Action(
                        session, args[0], ' '.join(args[1:]), data=data)
