    ...
    def parse_pgpmime(self, message):
        ...
        for part in message.walk():
            ...
            if (sig_count > 1) and (mimetype == 'application/pgp-signature'):
                ...
                try:
                    gpg = GnuPG(
                    ).run(['--utf8-strings', '--verify', sig.name, '-'],
                    ...
                    if not result:
                        ...
                        reslines = [g.split("gpg: ")[1]
                                    for g in result.strip().split("\n")]
                        matchgr = re.match(
                            ".*made (.*) using (.*) key ID ([a-zA-Z0-9]{8}).*", reslines[0])
                        keyid = matchgr.groups()[2]
                        keytype = matchgr.groups()[1]
                        datetime = matchgr.groups()[0]

                        # FIXME: This should understand what kind of UI we
                        # have.
                ...
                sig_alg = part.get_param(
                    'micalg', 'pgp-sha1').split('-')[1].upper()
                sig_count = 1
