...
def parse_rewritemap(args, os_settings, mailpiles=None):
    ...
    try:
        ...
        with open(APACHE_REWRITEMAP_PATH, 'r') as fd:
            for line in fd:
...
def save_rewritemap(args, os_settings, mailpiles):
    ...
    with open(APACHE_REWRITEMAP_PATH + '.new', 'w') as fd:
        os_settings['rewritemap'] = '\n'.join(rules)
        fd.write(APACHE_REWRITEMAP_TEMPLATE % os_settings)

    if os.path.exists(APACHE_REWRITEMAP_PATH):
        os.remove(APACHE_REWRITEMAP_PATH)

    os.rename(APACHE_REWRITEMAP_PATH + '.new', APACHE_REWRITEMAP_PATH)
...
def _get_mailpiles(args):
    ...
    mailpiles = {}
    parse_rewritemap(args, os_settings, mailpiles=mailpiles)
    if args.discover:
...
def list_mailpiles(args):
    os_settings = get_os_settings(args)
    mailpiles = parse_rewritemap(args, os_settings)
    discover_mailpiles(mailpiles=mailpiles)
...
def install_apache(app_args, args):
    if os.getuid() == 0:
        ...
        save_rewritemap(args, os_settings, _get_mailpiles(args))
...
def start_mailpile(app_args, args):
    ...
    if args.user:
        ...
        mailpiles[hostport] = (user_settings['user'],
                               ...
                               user_settings['port'],
                               False, None, None)
        save_rewritemap(args, os_settings, mailpiles)
